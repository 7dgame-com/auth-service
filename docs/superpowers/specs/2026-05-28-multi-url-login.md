...
当前服务已经具备标准 OAuth Authorization Code + PKCE 流程，并且 `OAuthClient.redirectUris` 会精确校验 `redirect_uri`。默认配置里已经出现 `bujiaban-web` 和 `3dugc-web` 两个 client，但“登录入口 URL”本身还没有成为一等配置：
...
- 微信网页授权 callback 使用单个 `AUTH_PUBLIC_BASE_URL` 拼接，服务实例只有一个公开 issuer/callback 域名。
- 菜单和 legacy 流程里仍有偏向单站点的默认行为。

这个 spec 设计一个可扩展模型，让不同业务 URL 都能通过统一认证服务登录，同时避免开放重定向、client 混淆和跨站 token 泄露。

## 目标

- 支持多个业务站点从不同 URL 发起登录，例如 `https://bujiaban.com/login`、`https://3dugc.com/login`、后续更多服务。
- 支持“短登录入口”，业务方可以跳到 `https://auth.bujiaban.com/login/:slug?...`，由认证服务补齐 client、默认 redirect URI、scope 和登录方式。
- 保持已有 `/oauth/authorize` 标准入口兼容，已经接 OAuth 的业务不需要改。
- 所有回跳 URL 必须可配置、可校验、可审计，不能接受任意外部 URL。
- 为后续微信网站扫码、小程序、更多品牌展示留接口，但第一阶段只落地公众号网页授权。
...
继续作为安全边界。一个 client 表示一个可被信任的业务应用，包含：
...
当前代码已有这个结构，后续只需要补充加载来源和校验能力。

### Login Entry

新增“登录入口”配置，负责把用户访问的登录 URL 映射到一个 OAuth client 和默认登录行为。建议结构：
...
5. 生成一个认证服务自己的 signed state，里面保存：
   - `clientId`
   - `redirectUri`
   - `returnTo`
   - `scopes`
...
建议不把 `return_to` 放进 OAuth `redirect_uri` 的 path/query 里动态改写，而是作为独立参数交给业务 callback 处理。这样 `redirect_uri` 可以继续精确匹配，安全边界更清楚。
...
但生产环境建议显式配置 `AUTH_LOGIN_ENTRIES_JSON`，避免新增域名时隐藏使用默认值。
